...
from lib.ghg.gs_parm_network import GSRegressor, AuxSimple,AuxRender512
...
import numpy as np 
# from lib.ghg.utils import get_input_calib
'''
SHARD BLOCK FEATURE SHAPEtorch.Size([1, 32, 1024, 1024])
rot_maps shape: torch.Size([1, 20, 1024, 1024])
scale_maps shape: torch.Size([1, 15, 1024, 1024])
opacity_maps shape: torch.Size([1, 5, 1024, 1024])
POST in_shell rot_maps shape: torch.Size([1, 4, 1024, 1024])
POST in_shell scale_maps shape: torch.Size([1, 3, 1024, 1024])
POST in_shell opacity_maps shape: torch.Size([1, 1, 1024, 1024])
'''
...
class GaussianRegressor(nn.Module):
    def __init__(self, cfg, with_gs_render=False):
        ...
        self.auxilary = AuxSimple()
        self.generator = GatedGenerator()
        for param in self.generator.parameters():
            param.requires_grad = False

    def forward(self, data, auxilary=False):
        ...
        with torch.no_grad(): #PINON: The gradients for inpainting would now not be tracked
        ...
        with autocast(enabled=self.cfg.raft.mixed_precision):
            ...
            data = self.decode_gsparms(appearance_map, appearance_feat, data, bs, valid, auxilary=auxilary)
    ...
    def decode_gsparms(self, appearance_map, appearance_feat, data, bs, valid, auxilary=False):
        ...
        # print(f"pos_offset_o shape {pos_offset_0.shape}")
        geometry_map = torch.concatenate([pos_offset_0,pos_offset_1,pos_offset_2,pos_offset_3],1)

        # print(f"Appearance map {appearance_map.shape}")
        # print(f"Geometry map {geometry_map.shape}")
        # print(f"Appearence Features {(appearance_feat[0].shape),(appearance_feat[1].shape),(appearance_feat[2].shape)}")
        rot_maps, scale_maps, opacity_maps, uv_features = self.gs_parm_regressor(
            appearance_map,
            geometry_map,
            appearance_feat)
        opacity_maps = opacity_maps.type(torch.float32)
        uv_features += appearance_map.clone().detach()
        
        # print(f"rot_maps shape: {rot_maps.shape}")
        # print(f"scale_maps shape: {scale_maps.shape}")
        # print(f"opacity_maps shape: {opacity_maps.shape}")
        for shell_idx, shell_name in enumerate(['in_shell','out_shell_1','out_shell_2','out_shell_3','out_shell_4']):
            ...
            data[shell_name]['features'] = uv_features[:,shell_idx*3:shell_idx*3+3, :, :]
           
        
        features = features2render(data,[0,0,0])
        input_viewlist = ['input_view_0', 'input_view_1', 'input_view_2']
        
        
        # print_memory_usage("Post Rendering")
        with autocast(enabled=True):            
            for idx,view in enumerate(input_viewlist):
                features_min = features[idx].min()
                features_max = features[idx].max()
                normalized_feature = 2.0 * (features[idx] - features_min) / (features_max - features_min) - 1.0
                
                
                recon_image = self.auxilary(normalized_feature) 
                                 
                data['input_view'][view]['recon_image'] = recon_image
        # print_memory_usage("Post Model")
