...
from lib.ghg.gs_parm_network import *
from lib.ghg.deep_fill_v2 import GatedGenerator
from lib.loss import sequence_loss
from lib.utils import repeat_interleave
from lib.utils import save_image , bytes_to_human, print_memory_usage
from torch import autocast

import torch.nn.functional as F
import cv2
import pdb
from lib.ghg.GaussianRender import features2render
...
class GaussianRegressor(nn.Module):
    def __init__(self, cfg, with_gs_render=False):
        ...
        self.dino_encoder = DinoV2FeatureExtractor().cuda()
        # self.dino_encoder.cuda()

        self.gs_parm_regressor = GSRegressorMC_Depth(self.cfg, rgb_dim=3, depth_dim=1)
        ...
        # if self.cfg.comet:
            # self.experiment = CometLogger.initialize_comet()
            # self.experiment.log_code(__file__)

    def forward(self, data, eval=False,total_steps=0):
        ...
        data['in_shell'] = {}
        # print_memory_usage("Before inpainting")
        ...
        # '''
        # print_memory_usage("Dont shoot the messenger :()")

        # data['in_shell']['rgb_maps'] = aggregated_image_2d
        ...
        with autocast(enabled=self.cfg.raft.mixed_precision,device_type='cuda',dtype=torch.float16):

            appearance_feat = self.img_encoder(appearance_map)

            data = self.decode_gsparms(appearance_map, appearance_feat, data, bs, valid,eval,total_steps)
    ...
    def decode_gsparms(self, appearance_map, appearance_feat, data, bs, valid,eval=False,total_steps=0):
        ...
        # print_memory_usage("Before primary network")
        
        dino_features  = self.dino_encoder(data['input_view']['img'].squeeze(0))
        # print(data['input_view']['img'].squeeze(0).shape)
        # print(appearance_feat[0].shape)
        # print(dino_features.shape)
        # exit()
        appearance_feat = list(appearance_feat)
        for idx,feat in enumerate(appearance_feat):            
            appearance_feat[idx] = feat + dino_features[idx]
        
        
        rot_maps, scale_maps, opacity_maps, color_maps, depth_maps = self.gs_parm_regressor(
            appearance_map,
            geometry_map,
            appearance_feat)
        ...
        # print_memory_usage("After primary network")
        ...
        for shell_idx, shell_name in enumerate(['in_shell','out_shell_1','out_shell_2','out_shell_3','out_shell_4']):
            ...
            data[shell_name]['rgb_maps'] = color_maps[:,shell_idx*3:shell_idx*3+3, :, :]
            # data[shell_name]['xyz'] = depth_maps[:,shell_idx*1:shell_idx*1+1, :, :]
            
            # Retrieve the original xyz tensor (shape: [bs, 1048576, 3])            
            data[shell_name]['xyz'] = data[shell_name]['xyz'] + depth_maps[:,shell_idx*3:shell_idx*3+3, :, :].permute(0, 2, 3, 1).view(bs, -1,3)
                   
         
            # data[shell_name]['input_rot_maps'] = input_rot_maps[:,shell_idx*4:shell_idx*4+4,:,:]
            # data[shell_name]['input_scale_maps'] = input_scale_maps[:,shell_idx*3:shell_idx*3+3, :, :]
            # data[shell_name]['input_opacity_maps'] = input_opacity_maps[:,shell_idx*1:shell_idx*1+1, :, :]

        

      
        # print_memory_usage("Before Rendering")
