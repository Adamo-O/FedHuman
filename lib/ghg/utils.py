...
def get_input_calib(data, opt, sample_name, subangle,view):
    ...
    data[view] = {}
    data[view]['FovX'] = torch.FloatTensor(np.array(fovx_list)).cuda()
    data[view]['FovY'] = torch.FloatTensor(np.array(fovy_list)).cuda()
    data[view]['world_view_transform'] = torch.concat(world_view_transform_list).cuda()
    data[view]['full_proj_transform'] = torch.concat(full_proj_transform_list).cuda()
    data[view]['camera_center'] = torch.concat(camera_center_list).cuda()
    return data
