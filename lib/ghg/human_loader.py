...
class HumanDataset(Dataset):
    def __init__(self, opt, phase='train', person_subset=None, personalized=False):
        self.opt = opt
        self.use_processed_data = opt.use_processed_data
        self.phase = phase
        self.dataset_name = 'THuman2.0'
        self.person_subset = person_subset  # List of person IDs to include in the dataset
        self.sample_to_person = {}  # Mapping from sample name to person ID
        self.sample_id_to_person = {}  # Mapping from sample ID to person ID
        ...
        if self.personalized:
            ...
            # Create a mapping from subject ID to person ID
            self.subject_to_person = {}
            for person_id, data in self.person_mapping.items():
                for subject_id in data[f'{self.phase}_ids']:
                    self.subject_to_person[subject_id] = person_id
                
            # Build the sample_to_person mapping for all samples
            self.sample_id_list = []
            self.sample_id_to_name = {}
            for sample_name in self.sample_list:
                subject_id = sample_name.split('_')[0] if '_' in sample_name else sample_name
                if subject_id in self.subject_to_person:
                    self.sample_to_person[sample_name] = self.subject_to_person[subject_id]
                    self.sample_id_list.append(subject_id)
                    
                    if subject_id not in self.sample_id_to_name:
                        self.sample_id_to_name[subject_id] = []
                    self.sample_id_to_name[subject_id].append(sample_name)
        
            # Remove duplicates
            self.sample_id_list = list(set(self.sample_id_list))
            
            # Filter the sample_list to only include samples from the specified persons if person_subset is provided
            if self.person_subset is not None:
                filtered_sample_list = []
                for sample_id in self.sample_id_list:
                    sample_names = self.sample_id_to_name[sample_id]
                    for sample_name in sample_names:
                        # print(f"sample_name: {sample_name}, person_id: {self.sample_to_person[sample_name]}, is name in person_map: {sample_name in self.sample_to_person} is in person_subset: {self.sample_to_person[sample_name] in self.person_subset}")
                        if sample_name in self.sample_to_person and self.sample_to_person[sample_name] in self.person_subset:
                            filtered_sample_list.append(sample_name)
                
                # Check if we have any samples after filtering
                if len(filtered_sample_list) == 0:
                    logging.warning(f"No samples found for the specified person subset. Using all samples instead.")
                else:
                    self.sample_list = filtered_sample_list
                    logging.info(f"Filtered sample list to {len(self.sample_list)} samples from {len(self.person_subset)} persons")
            
            # logging.info(f"Sample to person mapping: {self.sample_to_person}")
            # logging.info(f"Sample list: {self.sample_list}")
    ...
    def get_person_id(self, sample_name):
        """Get the person ID for a given sample name."""
        if self.personalized:
            return self.sample_to_person.get(sample_name)
        return None
    ...
    def get_item(self, index, novel_id=None):
        ...
        # # Only add person_id to the returned data if personalized mode is enabled
        # if self.personalized:
        #     person_id = self.get_person_id(sample_name)
        #     dict_tensor['person_id'] = torch.tensor(person_id, dtype=torch.long)
